...
def bartloss(instruction, input_context, output):
    received = False
    while not received:
        try:
            result = bart_scorer.score([instruction+input_context], [output], batch_size=1)
            received = True
        except:
            time.sleep(1)
    score = result[0]
    ...
    for epoch in range(epochs):
        ...
        for item in train_data:
            ...
            for k in range(sample_size):
                ...
                loss = bartloss(instruction, input_context, hypo_output)
...
def test(sample_size=3):
    ...
    for item in test_data:
        ...
        for k in range(sample_size):
            ...
            loss = bartloss(instruction, input_context, output)
            origin_loss = bartloss(instruction, input_context, output_origin)
            loss_list.append(loss)
