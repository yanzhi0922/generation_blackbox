...
def pmi():
    ...
    result = []
    if args.file_name:
        with open("./pmi/" + "pmi_" + args.file_name.lower() + "_gpt" + ".txt", 'r', encoding='utf-8') as f:
            for line in f:
                result.append(line.strip('\n'))
    elif args.task_name:
        with open("./pmi/" + "pmi_" + args.task_name.lower() + "_gpt" + ".txt", 'r', encoding='utf-8') as f:
            for line in f:
                result.append(line.strip('\n'))
    unique = []
...
def create_batches(dataset, batch_size=1, shuffle=False):
    if isinstance(dataset, dict): # 如果是dataset字典
    ...
    if shuffle:
        dataset_dict = pd.DataFrame(dataset_dict)  # 转换为DataFrame
...
def complete_api(prompt, l, model_name, temp=0.5, num_log_probs=10, echo=False, n=None):
    response = None
    received = False
    while not received:
        try:
            client = OpenAI(api_key="sk-0c2e4c0ec7444bc7924a645788c4dd24", base_url="https://api.deepseek.com/beta")
            response = client.completions.create(model="deepseek-chat",
                                                 prompt=prompt,
                                                 max_tokens=l,
                                                 temperature=temp,
...
def get_regular_label_probs(response, batch, labels, args, if_null=True, split="train"):
    ...
    if len(all_missing_positions) > 0:
        ...
        batches = create_batches(additional_dataset,1)

        for m, missing_batch in enumerate(batches['sentence']):
            if split == "train":
                tmp_ = missing_batch[0]
                print("tmp_", tmp_)
                missing_response = complete_api(tmp_, l=0, model_name=args.model_name_or_path,
                                                     num_log_probs=1, echo=True)
...
def evaluate(args, eval_batches, metric, ce_loss, accelerator, epoch, best_epoch, best_result, results,
             prompts_probs=None, prompt_length=None, tokenizer=None, linear_layer=None, prompts=None,
             prompt_embedding_fc=None, label_to_id=None):
    ...
    for step in range(len(eval_batches['sentence'])):
        ...
        tmp = batch[0]
        print("evaluate_tmp", tmp)
        responses = complete_api(tmp, l=1, model_name=args.model_name_or_path, num_log_probs=10, echo=False, n=None)
...
def test(args, test_batches, metric, accelerator, epoch, results, prompts_probs=None, prompt_length=None,
         tokenizer=None, linear_layer=None, prompts=None, label_to_id=None, test_batches_mm=None):
    if args.task_name == None or args.k_shot >= 0:
        ...
        for step in range(len(test_batches['sentence'])):
            ...
            tmp = batch[0]
            print("test_tmp", tmp)
            responses = complete_api(tmp, l=1, model_name=args.model_name_or_path, num_log_probs=100, echo=False,
                                     n=None)

            label_keys = list(label_to_id.keys())
            converted_target = torch.tensor([label_to_id[i] for i in label])
            label_probs = get_regular_label_probs(responses, batch, label_keys, args, if_null=True,
                                                  split="test")  # logits_only : True
            logits = label_probs.squeeze()
            predictions = logits.argmax(dim=-1)
            if len(predictions.shape) == 0: predictions = predictions.unsqueeze(0)
...
if __name__ == "__main__":
    ...
    hinge_loss = MarginLoss(margin=args.margin, target=False)
    ...
    def preprocess_function(examples):
        ...
        for j in range(len(examples[sentence1_key])):
            if sentence2_key is None:
                ori_sent_id = tokenizer.tokenize(examples[sentence1_key][j])[:400]
                new_sent = tokenizer.convert_tokens_to_string(ori_sent_id)
                result["input"].append('input: ' + new_sent + template_cfg + "\n" + "output:")
            else:
                result["input"].append(
                    'input: sentence one: ' + examples[sentence1_key][j] + ' sentence two: ' + examples[sentence2_key][
                        j] + template_cfg + "\n" + "output:")
    ...
    train_batches = create_batches(train_dataset, batch_size=1, shuffle=True)
    eval_batches = create_batches(eval_dataset, batch_size=1)
    test_batches = create_batches(test_dataset, batch_size=1)
    if args.task_name == 'mnli':
    ...
    for epoch in range(args.num_train_epochs):
            ...
            for step in range(len(train_batches['sentence'])):
                ...
                with torch.no_grad():
                    ...
                    for k in range(args.sample_size):
                        prompts_discrete_indices = prompts_dist.sample()  # .sample()
                        prompts_discrete_indices_list.append(prompts_discrete_indices)
                        if args.use_ngram:
                            prompts_discrete_ngram_list = []
                            indices_list = prompts_discrete_indices.int().tolist()  # 将tensor转换为list
                            for idx in indices_list:
                                prompts_discrete_ngram_list.append(ngram_list[idx])
                            prompts_discrete = ' '.join(prompts_discrete_ngram_list)
                        else:
                        ...
                        for i in range(len(train_batches['sentence'][step])):
                            ...
                            print(train_batches['sentence'][step][i])

                        tmp = batch[0]
                        print(tmp)
                        responses = complete_api(tmp, l=1, model_name=args.model_name_or_path, num_log_probs=10,
                                                      echo=False, n=None)
                        print(responses)
                        time.sleep(2)
                        label_keys = list(label_to_id.keys())
                        converted_target = torch.tensor([label_to_id[label] for label in labels])  # 将list转换为tensor
                        ...
                        if complete_api.count >= args.api_limit:
        ...
        eval_result = evaluate(args, eval_batches, metric, ce_loss, accelerator, epoch, best_epoch, best_eval_result,
                               eval_results, prompts_probs=prompts_probs, prompt_length=prompt_length,
                               tokenizer=tokenizer, linear_layer=None, prompts=None, prompt_embedding_fc=None,
    ...
    test(args, test_batches, metric, accelerator, epoch, test_results, prompts_probs=best_prompts_probs,
         prompt_length=prompt_length, tokenizer=tokenizer, linear_layer=None, prompts=None, label_to_id=label_to_id,
         test_batches_mm=test_batches_mm)